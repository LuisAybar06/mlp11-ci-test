...
def check_blank_lines_blocks(file_path):
    errors=0
        ...
        for i in range(2, len(lines)):
            ...
            if re.match(r'^\s*(class|def|if|for|while|with)', lines[i]):
                ...
                if not re.match(r'^\s*$', lines[i-1]) or not re.match(r'^\s*$', lines[i-2]):
                    ...
                    print(f"{file_path}:{i+1}: Expected exactly 2 blank lines before block definition, tu tienes mas de tres")
                    errors += 1
    return errors
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if file.endswith('.py'):
                ...
                total_errors += check_blank_lines_blocks(file_path)
...
if __name__=="__main__":
    ...
    parser.add_argument("directories", nargs='+', help="Direstories to lint")
